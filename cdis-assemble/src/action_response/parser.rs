use crate::action_response::model::{ActionResponse, ActionResponseFieldsPresent};
...
pub(crate) fn action_response_body(input: BitInput) -> IResult<BitInput, CdisBody> {
    ...
    let (input, request_status) = uvint32(input)?;

    let (input, number_of_fixed_datums) = parse_field_when_present(
        fields_present,
        ActionResponseFieldsPresent::FIXED_DATUMS_BIT,
        uvint8,
    )(input)?;
    ...
    let (input, number_of_var_datums) = parse_field_when_present(
        fields_present,
        ActionResponseFieldsPresent::VARIABLE_DATUMS_BIT,
        uvint8,
    )(input)?;
