...
pub(crate) fn transmitter_body(input: BitInput) -> IResult<BitInput, CdisBody> {
    ...
    let (input, radio_type) = parse_field_when_present(
        fields_present,
        TransmitterFieldsPresent::RADIO_TYPE_BIT,
        entity_type,
    )(input)?;
    ...
    let (input, antenna_location) = parse_field_when_present(
        fields_present,
        TransmitterFieldsPresent::ANTENNA_LOCATION_BIT,
        world_coordinates,
    )(input)?;
    let (input, relative_antenna_location) = parse_field_when_present(
        fields_present,
        TransmitterFieldsPresent::RELATIVE_ANTENNA_LOCATION_BIT,
        entity_coordinate_vector,
    )(input)?;

    let (input, antenna_pattern_type): (BitInput, Option<u16>) = parse_field_when_present(
        fields_present,
        TransmitterFieldsPresent::ANTENNA_PATTERN_BIT,
        take(THREE_BITS),
    )(input)?;
    let antenna_pattern_type = antenna_pattern_type.map(TransmitterAntennaPatternType::from);

    let (input, _antenna_pattern_length): (BitInput, Option<u16>) = parse_field_when_present(
        fields_present,
        TransmitterFieldsPresent::ANTENNA_PATTERN_BIT,
        take(TEN_BITS),
    )(input)?;

    let (input, frequency) = if field_present(
        fields_present,
    ...
    let (input, transmit_frequency_bandwidth) = if field_present(
        fields_present,
        TransmitterFieldsPresent::TRANSMITTER_DETAILS_BIT,
    ) {
    ...
    let (input, power): (BitInput, Option<u8>) = parse_field_when_present(
        fields_present,
        TransmitterFieldsPresent::TRANSMITTER_DETAILS_BIT,
        take(EIGHT_BITS),
    )(input)?;

    let (input, modulation_type) = if field_present(
        fields_present,
        TransmitterFieldsPresent::TRANSMITTER_DETAILS_BIT,
    ) {
    ...
    let (input, crypto_system): (BitInput, Option<u16>) = parse_field_when_present(
        fields_present,
        TransmitterFieldsPresent::CRYPTO_DETAILS_BIT,
        take(FOUR_BITS),
    )(input)?;
    let crypto_system = crypto_system.map(TransmitterCryptoSystem::from);
    let (input, crypto_key_id): (BitInput, Option<u16>) = parse_field_when_present(
        fields_present,
        TransmitterFieldsPresent::CRYPTO_DETAILS_BIT,
        take(SIXTEEN_BITS),
    )(input)?;
    ...
    let (input, length_of_modulation_parameters): (BitInput, Option<u16>) =
        parse_field_when_present(
            fields_present,
            TransmitterFieldsPresent::MODULATION_PARAMETERS_BIT,
            take(EIGHT_BITS),
        )(input)?;

    let (input, modulation_parameters) = if field_present(
        fields_present,
        TransmitterFieldsPresent::MODULATION_PARAMETERS_BIT,
    ) {
    ...
    let (input, antenna_pattern) = if field_present(
        fields_present,
        TransmitterFieldsPresent::ANTENNA_PATTERN_BIT,
    ) {
    ...
    let (input, variable_transmitter_parameters) = if field_present(
        fields_present,
        TransmitterFieldsPresent::VARIABLE_PARAMETERS_BIT,
    ) {
...
fn variable_transmitter_parameter(
    input: BitInput,
) -> IResult<BitInput, VariableTransmitterParameter> {
